...
@auto_register("system_rank")
class SystemRank(Bench):
    bench_name = "system_rank"
    ...
    def build_format_data(self, data_type, raw_file, format_file):
        ...
        question_dict = {}
        for line in open("data/fennec_eval_data/raw/system_rank/question.jsonl").readlines():
            json_item = json.loads(line)
            question_dict[json_item['question_id']] = json_item['turns'][0]
        ...
        with open(format_file, "w") as fw:
            ...
            for line in open(raw_file).readlines():
                ...
                if data_type == "pairwise":
                    ...
                    if "test" in format_file:
                        meta_info = {
                            "question_id": line_idx,
                            "raw_question_id": json_data['question_id'],
                            "model_a": "",
                            "model_b": "",
                            "category": json_data["model_id"],
                            "judge": [0],
                            "judge_type": "pairwise",
                            "judgment": [""],
                            "turn": 1,
                        ...
                        conversation_a = [
                            {
                                "content": question_dict[json_data['question_id']],
                                "role": "user",
                            },
                            {
                                "content": json_data['choices'][0]['turns'][0],
                                "role": "assistant",
                            },
                        ]
                        conversation_b = [
